    ...
    for episode in range(num_iter):
        ...
        while True:
            ...
            if tau >= update_state:     # update_state 시작위치부터 n개를 reward_trace[]에서 가져와야 하기 때문
                ...
                N[state_trace[update_state], n] += alpha * (G - N[state_trace[update_state], n])
...
if __name__ == '__main__':
    ...
    for state in env.STATES:
        ...
        for n in range(1, STEP_N_MAX+1):
            step_n_values.append(N[(state, n)])
        print(state, step_n_values.index(max(step_n_values))+1)
    ...
    plt.savefig('images/n_step_variable[state_dependency]_alpha_0,2.png')
    plt.close()
