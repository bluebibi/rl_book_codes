...
class TIC_TAC_TOE_ENV:
    ...
    def play(self, print_board=False):
        ...
        if print_board:
            self.board.initial_state.print_board()

        # 게임 진행
        current_state = self.board.initial_state
        while True:
            ...
            next_state_hash = self.board.get_new_state(i, j, current_state.data, player_int).hash()
...
def train(epochs, print_every_n=500):
    ...
    player1 = Player(env.board, epsilon=epsilon)
    player2 = Player(env.board, epsilon=epsilon)
    ...
    for i in range(1, epochs + 1):
        winner = env.play(print_board=False)
...
def self_play(turns):
    ...
    player1 = Player(env.board, epsilon=0)
    player2 = Player(env.board, epsilon=0)
    env.reset(player1, player2)
...
def draw_figure_after_train(episode_list, tie_rate_list, player1_win_rate_list, player2_win_rate_list):
    ...
    plt.ylabel('비율')
    plt.legend()
    plt.savefig('images/tic_tac_toe_train.png')
    plt.show()
    plt.close()
